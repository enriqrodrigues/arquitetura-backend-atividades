...
class Horario:
    ...
    def __post_init__(self):
        ...
        # Extrai apenas a parte de horário (horas, minutos, segundos)
        t_inicio = self.inicio.time()
        t_fim = self.fim.time()

        # Limites do horário comercial
        hora_inicio = time(8, 0)
        hora_fim = time(17, 0)

        # Verifica se o intervalo está inteiramente dentro da manhã ou da tarde
        hora_comercial = (hora_inicio <= t_inicio <= hora_fim) and (hora_inicio <= t_fim <= hora_fim)

        if not (hora_comercial):
            raise ValueError("Horários inválidos. Favor informar um horário entre 8h e 17h.")
